...
int main(int argc, char *argv[]){
	...
	s_output= "newoutput"+ std::string(argv[1]) + "_"+std::string(argv[2])
				+"_"+std::string(argv[3])+".txt";
	s_squars="newsquars2_"+std::string(argv[3])+".txt";
	Run run(s, s_squars);
	...
	//~ for (double d=0.1; d<=0.1; d+=0.01){
		//~ for (int T=0; T<=1500;T = T+ 1){
			//~ for (double A=atof(argv[1]); A<=atof(argv[2]); A=A+0.5){
				//~ int mark=0;
				...
				//~ if (mark==0){
					//~ //output<<T<<" "<<A<<" 3"<<endl;
					//~ cout<<T<<" "<<A<<endl;
					//~ output<<T<<" "<<A<<" "<<run.multip_test(T,A,d,rep)<<endl;
				//~ }
			//~ }
		//~ }
	//~ }
	//for (double d=0.1; d<=0.1; d+=0.01){
		for (int T=atoi(argv[1]); T<=atoi(argv[2]);T = T+ 50){
